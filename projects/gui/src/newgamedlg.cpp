      ...
      ui(new Ui::NewGameDialog)
{
    ...
    connect(ui->m_configureRedEngineButton, SIGNAL(clicked()),
    ...
    connect(ui->m_redEngineComboBox, SIGNAL(currentIndexChanged(int)),
            this, SLOT(onEngineChanged(int)));
    connect(ui->m_blackEngineComboBox, SIGNAL(currentIndexChanged(int)),
            this, SLOT(onEngineChanged(int)));

    ui->m_redEngineComboBox->setModel(m_proxyModel);
    ui->m_redEngineComboBox->setValidator(engineValidator);
    ...
    connect(ui->m_redPlayerCpuRadio, &QRadioButton::toggled, [=](bool checked)
    {
...
PlayerBuilder* NewGameDialog::createPlayerBuilder(Chess::Side side) const
{
    ...
    bool ignoreFlag = QSettings().value("games/human_can_play_after_timeout",
                                        true).toBool();
    return new HumanBuilder(SylvanApplication::userName(), ignoreFlag);
}
...
void NewGameDialog::setPlayerType(Chess::Side side, PlayerType type)
{
    ...
    {
        ui->m_redEngineComboBox->setEnabled(type == CPU);
        ui->m_configureRedEngineButton->setEnabled(type == CPU);
    }
    else
    {
...
NewGameDialog::PlayerType NewGameDialog::playerType(Chess::Side side) const
{
    ...
    if (side == Chess::Side::Red)
        return (ui->m_redPlayerHumanRadio->isChecked()) ? Human : CPU;
    else
...
{
    Chess::Side side;
    if (QObject::sender() == ui->m_configureRedEngineButton)
        side = Chess::Side::Red;
    else
...
void NewGameDialog::onVariantChanged(const QString& variant)
{
    ...
    if (empty)
    {
        ui->m_redPlayerHumanRadio->setChecked(true);
        ui->m_blackPlayerHumanRadio->setChecked(true);
    }

    ui->m_redPlayerCpuRadio->setDisabled(empty);
...
void NewGameDialog::onEngineChanged(int index, Chess::Side side)
{
    ...
    {
        if (QObject::sender() == ui->m_redEngineComboBox)
            side = Chess::Side::Red;
        else
            side = Chess::Side::Black;
    }
